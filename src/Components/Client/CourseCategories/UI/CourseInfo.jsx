...
const CourseInfo = ({ fusername, rollno }) => {


    const { id } = useParams();

    const [UIcoursedetail, set_UIcoursedetail] = useState([]);
    const [UIFeeback, set_UIFeeback] = useState([]);
    ...
    useEffect(() => {
        axios.get(`http://localhost:9002/courses/UI/${id}`)
            .then(response => {
                const { course, feedback } = response.data;
                set_UIcoursedetail(course);
                set_UIFeeback(feedback)
            })
            .catch(error => {
                console.log(error);
            });
    ...
    function handleSubmit() {
        const course_id = UIcoursedetail[0].course_id;
        const category_id = UIcoursedetail[0].category_id;
        const data = { feedback, course_id, category_id, fusername };
        axios.post(`http://localhost:9002/courses/UI/${id}`, data)
            .then(res => {
    ...
    const handleDownload = async () => {
        ...
        try {
          ...
          const course_id = UIcoursedetail[0].course_id;
          const category_name = UIcoursedetail[0].category_name;
          const course_name = UIcoursedetail[0].course_name;
                                                ...
                                                <div className="description-tab ">
                                                    ...
                                                    <div className="tab-content" id="myTabContent">
                                                        ...
                                                        <div className="tab-pane fade mt-4" id="reviews" role="tabpanel" aria-labelledby="reviews-tab">
                                                            <h6 style={{ fontSize: "22px" }} className="">Student Feedback</h6>
                                                            {
                                                                UIFeeback.map((val, key) => {
                                                                    return <div id={key}>
                                                                        <h6>{val.user_name}</h6>
