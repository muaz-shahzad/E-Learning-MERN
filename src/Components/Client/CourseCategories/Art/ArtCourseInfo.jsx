...
const ArtCourseInfo = ({ fusername,rollno }) => {
    const { id } = useParams();
    console.log("Detail prams id", id)

    const navigate = useNavigate();

    const [Artcoursedetail, set_artcoursedetail] = useState([]);
    const [ArtFeeback, set_ArtFeeback] = useState([]);
    ...
    useEffect(() => {
        axios.get(`http://localhost:9002/courses/art/${id}`)
            .then(response => {
                const { course, feedback } = response.data;
                set_artcoursedetail(course);
                set_ArtFeeback(feedback)
            })
            .catch(error => {
                console.log(error);
            });
    }, []);
    console.log(Artcoursedetail)

    function handleSubmit() {
        const course_id = Artcoursedetail[0].course_id;
        const category_id = Artcoursedetail[0].category_id;
        const data = { feedback, course_id, category_id, fusername };
        axios.post(`http://localhost:9002/courses/art/${id}`, data)
            .then(res => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const course_id = Artcoursedetail[0].course_id;
            const category_name = Artcoursedetail[0].category_name;
            const course_name = Artcoursedetail[0].course_name;
