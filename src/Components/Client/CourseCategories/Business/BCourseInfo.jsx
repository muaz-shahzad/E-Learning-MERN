...
const BCourseInfo = ({ fusername,rollno }) => {
    const { id } = useParams();
    console.log("Detail prams id", id)

    const navigate = useNavigate();

    const [Bcoursedetail, set_Bcoursedetail] = useState([]);
    const [BFeeback, set_BFeeback] = useState([]);
    ...
    useEffect(() => {
        axios.get(`http://localhost:9002/courses/buisness/${id}`)
            .then(response => {
                const { course, feedback } = response.data;
                set_Bcoursedetail(course);
                set_BFeeback(feedback)
            })
            .catch(error => {
                console.log(error);
            });
    }, []);
    console.log(Bcoursedetail)


    
    function handleSubmit() {
        const course_id = Bcoursedetail[0].course_id;
        const category_id = Bcoursedetail[0].category_id;
        const data = { feedback, course_id, category_id, fusername };
        axios.post(`http://localhost:9002/courses/buisness/${id}`, data)
            .then(res => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const course_id = Bcoursedetail[0].course_id;
            const category_name = Bcoursedetail[0].category_name;
            const course_name = Bcoursedetail[0].course_name;
