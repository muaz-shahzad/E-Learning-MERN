...
const HealthCourseInfo = ({ fusername,rollno }) => {
    const { id } = useParams();
    console.log("Detail prams id", id)

    const navigate = useNavigate();

    const [Healthcoursedetail, set_Healthcoursedetail] = useState([]);
    const [HealthFeeback, set_HealthFeeback] = useState([]);
    ...
    useEffect(() => {
        axios.get(`http://localhost:9002/courses/health/${id}`)
            .then(response => {
                const { course, feedback } = response.data;
                set_Healthcoursedetail(course);
                set_HealthFeeback(feedback)
            })
            .catch(error => {
                console.log(error);
            });
    }, []);
    console.log(Healthcoursedetail)
    function handleSubmit() {
        const course_id = Healthcoursedetail[0].course_id;
        const category_id = Healthcoursedetail[0].category_id;
        const data = { feedback, course_id, category_id, fusername };
        axios.post(`http://localhost:9002/courses/health/${id}`, data)
            .then(res => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const course_id = Healthcoursedetail[0].course_id;
            const category_name = Healthcoursedetail[0].category_name;
            const course_name = Healthcoursedetail[0].course_name;
                    ...
                    <div className="container">
                        <div className="row">
                            <div className="col-lg-12 mb-5">
                                {
                                    Healthcoursedetail.map((val, key) => {
...
export default HealthCourseInfo