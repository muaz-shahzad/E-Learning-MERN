...
const GCourseInfo = ({ fusername,rollno }) => {
    const { id } = useParams();
    console.log("Detail prams id", id)

    const navigate = useNavigate();

    const [GUIcoursedetail, set_GUIcoursedetail] = useState([]);
    const [GUIFeeback, set_GUIFeeback] = useState([]);
    ...
    useEffect(() => {
        axios.get(`http://localhost:9002/courses/graphic/${id}`)
            .then(response => {
                const { course, feedback } = response.data;
                set_GUIcoursedetail(course);
                set_GUIFeeback(feedback)
            })
            .catch(error => {
                console.log(error);
            });
    }, []);
    console.log(GUIcoursedetail)
    function handleSubmit() {
        const course_id = GUIcoursedetail[0].course_id;
        const category_id = GUIcoursedetail[0].category_id;
        const data = { feedback, course_id, category_id, fusername };
        axios.post(`http://localhost:9002/courses/graphic/${id}`, data)
            .then(res => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const course_id = GUIcoursedetail[0].course_id;
            const category_name = GUIcoursedetail[0].category_name;
            const course_name = GUIcoursedetail[0].course_name;
